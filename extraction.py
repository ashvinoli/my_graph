...
def extract(raw_string):
	...
	while i < len(tokens)-1:
		if (re.match(pattern_num,tokens[i]) and (re.match(pattern_alpha,tokens[i+1]) or re.match(pattern_trig,tokens[i+1]) or re.match("\(",tokens[i+1]) )) or (re.match(pattern_trig,tokens[i]) and re.match(pattern_trig,tokens[i+1])) or (re.match(pattern_alpha,tokens[i]) and re.match(pattern_alpha,tokens[i+1])) or (re.match("\)",tokens[i]) and re.match(pattern_alpha,tokens[i+1])):
